...
import {
  FamilyDocument,
  subscribeToDocumentsInFolder,
  uploadDocument,
  deleteDocument,
} from '../../services/documentService';
import {HomeStackParamList} from '../../types';

type Props = NativeStackScreenProps<HomeStackParamList, 'FolderScreen'>;
...
function DocRow({doc, canDelete, accentColor, onDelete}: DocRowProps) {
  ...
  return (
    <Swipeable renderRightActions={renderRight}>
      <TouchableOpacity
        ...
        <Text style={[styles.openArrow, {color: colors.textTertiary}]}>›</Text>
      </TouchableOpacity>
    </Swipeable>
  );
}

export default function FolderScreen({route}: Props) {
  const {folderId, folderName, folderColor, folderCreatedBy} = route.params;
  const {colors} = useTheme();
  ...
  // For private folders, only the owner can upload
  const canUpload = isAdmin || folderCreatedBy === uid || role !== 'child';
  ...
  useEffect(() => {
    if (!family) {return;}
    const unsub = subscribeToDocumentsInFolder(family.id, folderId, setDocuments);
    return unsub;
  }, [family, folderId]);
  ...
  async function handleDelete(doc: FamilyDocument) {
    if (!family) {return;}
    Alert.alert('Delete File', `Remove "${doc.name}"?`, [
  ...
  return (
    <View style={[styles.container, {backgroundColor: colors.background}]}>
      {/* Folder colour accent strip */}
      <View style={[styles.accentStrip, {backgroundColor: folderColor}]} />
        ...
        ListEmptyComponent={
          <View style={[styles.emptyCard, {backgroundColor: colors.surface}]}>
            ...
            <Text style={[styles.emptySub, {color: colors.textSecondary}]}>
              {canUpload
                ? `Tap + to upload files to ${folderName}`
      ...
      {uploading && (
        <View style={styles.uploadOverlay}>
          <View style={[styles.uploadCard, {backgroundColor: colors.surface}]}>
            <ActivityIndicator color={folderColor} size="large" />
      ...
      {canUpload && (
        <TouchableOpacity
          style={[
            styles.fab,
            {backgroundColor: folderColor, bottom: insets.bottom + 30},
          ]}
