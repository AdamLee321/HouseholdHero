...
export default function TodosScreen() {
  ...
  async function handleCreate() {
    ...
    setCreating(true);
    try {
      await createTodoList(family.id, title, type, uid, profile?.displayName ?? 'Someone');
      setTitle('');
      setType('shared');
    ...
    return (
      <Swipeable
        ...
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate('TodoList', {listId: list.id, title: list.title})}
          activeOpacity={0.7}>
          <View style={styles.rowLeft}>
            <Text style={[styles.rowTitle, {color: colors.text}]}>{list.title}</Text>
            <Text style={[styles.rowMeta, {color: colors.textTertiary}]}>
              {list.itemCount} item{list.itemCount !== 1 ? 's' : ''} · {list.createdByName}
            </Text>
          </View>
          <View style={[
            styles.typeBadge,
            {backgroundColor: list.type === 'shared' ? colors.primaryLight : colors.surfaceSecondary},
          ]}>
            <Text style={[
              styles.typeBadgeText,
              {color: list.type === 'shared' ? colors.primary : colors.textSecondary},
            ]}>
  ...
  const sections = [
    ...(shared.length > 0 ? [{type: 'header', label: 'SHARED'}, ...shared.map(l => ({type: 'list', list: l}))] : []),
    ...(personal.length > 0 ? [{type: 'header', label: 'PERSONAL'}, ...personal.map(l => ({type: 'list', list: l}))] : []),
  ];
  ...
  return (
    <View style={[styles.container, {backgroundColor: colors.background}]}>
      {loading ? (
        ...
        <FlatList
          data={sections}
          keyExtractor={(item, i) => item.type === 'header' ? `header-${i}` : (item as any).list.id}
          renderItem={({item}) => {
            if (item.type === 'header') {
              return (
      ...
      <Modal
        ...
        <KeyboardAvoidingView
          ...
          <View style={[styles.modalSheet, {backgroundColor: colors.surface, paddingBottom: insets.bottom + 16}]}>
            ...
            {/* Type toggle */}
            <View style={[styles.typeToggle, {backgroundColor: colors.surfaceSecondary}]}>
              {(['shared', 'personal'] as const).map(t => (
                <TouchableOpacity
                  key={t}
                  style={[styles.typeOption, type === t && {backgroundColor: colors.surface}]}
                  onPress={() => setType(t)}>
                  <Text style={[styles.typeOptionText, {color: type === t ? colors.primary : colors.textTertiary}]}>
            ...
            <TouchableOpacity
              style={[styles.addBtn, {backgroundColor: colors.primary}, !title.trim() && {opacity: 0.5}]}
...
const styles = StyleSheet.create({
  ...
  typeBadge: {paddingHorizontal: 10, paddingVertical: 4, borderRadius: 20, marginRight: 8},
  ...
  typeToggle: {flexDirection: 'row', borderRadius: 10, padding: 4, marginBottom: 12},
