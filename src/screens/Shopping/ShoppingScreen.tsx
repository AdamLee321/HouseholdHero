...
export default function ShoppingScreen() {
  ...
  const allItems = [...unchecked, ...(checked.length > 0 ? [{id: '__divider__'} as any] : []), ...checked];

  async function handleAdd() {
    if (!itemName.trim() || !family) {return;}
    setAdding(true);
    try {
  ...
  async function handleClearChecked() {
    if (!family || checked.length === 0) {return;}
    Alert.alert(
      'Clear Checked',
      `Remove ${checked.length} checked item${checked.length !== 1 ? 's' : ''}?`,
      [
        {text: 'Cancel', style: 'cancel'},
        {text: 'Clear', style: 'destructive', onPress: () => clearCheckedItems(family.id)},
      ],
    );
  }
    ...
    if (item.id === '__divider__') {
      return (
        <View style={styles.dividerRow}>
          ...
          <Text style={[styles.dividerLabel, {color: colors.textTertiary}]}>Checked</Text>
    ...
    return (
      <Swipeable
        ...
        <TouchableOpacity
          ...
          onPress={() => family && toggleShoppingItem(family.id, item.id, item.checked)}
          activeOpacity={0.7}>
          {/* Checkbox */}
          <View style={[
            styles.checkbox,
          ...
          <View style={styles.itemText}>
            ...
            <Text style={[styles.itemMeta, {color: colors.textTertiary}]}>
              {item.quantity ? `${item.quantity} · ` : ''}{item.addedByName}
            </Text>
          </View>
        </TouchableOpacity>
      </Swipeable>
      ...
      <Modal
        ...
        <KeyboardAvoidingView
          ...
          <View style={[styles.modalSheet, {backgroundColor: colors.surface, paddingBottom: insets.bottom + 16}]}>
            ...
            <TouchableOpacity
              style={[styles.addBtn, {backgroundColor: colors.primary}, !itemName.trim() && {opacity: 0.5}]}
              onPress={handleAdd}
